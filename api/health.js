...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ─────────────────────────────────────────
      ...
      //             특별시"로 개편됐어요. "전라남도"/"광주광역시"로 보내면 0건이지만, 이 API는
      //             아직 "전남" 하나로 옛 전남+광주 지역을 전부 담고 있는 걸 실제 테스트로
      //             확인해서(광주 소재 병원도 "전남"으로 검색하면 나옴) 그 값을 씀.
      const EMERGENCY_SIDO_RENAME = { '강원도': '강원특별자치도', '전라북도': '전북특별자치도', '전라남도': '전남', '광주광역시': '전남' };
